...
def rdspec(filename=None):
    ...
    if (base.find("mastar-") > -1):
        ...
        if np.sum(bad) > 0:
            ...
            err = 1.0/np.sqrt(ivar)
        spec.err = err
        spec.ivar = tab["IVAR"].data
        spec.wave = tab["WAVE"].data
        spec.mask = tab["MASK"].data
        spec.disp = tab["DISP"].data
        spec.presdisp = tab["PREDISP"].data
        ...
        # flatten arrays since some could be in the form of (1,n_pixel)
        spec.flux = spec.flux.ravel()
        spec.wave = spec.wave.ravel()
        spec.err = spec.err.ravel()
        spec.mask = spec.mask.ravel()
